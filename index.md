...
[sfext](https://github.com/elipousson/sfext) is built around existing sf
functions but designed to offer greater flexibility around both inputs
...
and offers a similar functionality:
...
However,
[`read_sf_ext()`](https://elipousson.github.io/sfext/reference/read_sf_ext.md)
also supports URLs for Google Sheets, FeatureLayers, data included with
an installed package, and a variety of other sources. The function also
supports an optional bounding box filter.

``` r

read_sf_ext("https://carto.nationalmap.gov/arcgis/rest/services/govunits/MapServer/29", bbox = as_bbox(nc))
#> ── Downloading "National Park" from <
...
but optionally preserve a name column (collapsing the values of that
...
but accepts bounding box objects as an input, allows you to set the
units for the buffer distance using a character string (automatically
converts the buffer distance units to match the units of the input
object):
...
but makes it easy to set the dimensions of the grid using rows, columns,
and an overall aspect ratio:
...
reference system of the output (using
...
The crs parameter also supports sf, sfc, or bbox inputs. Functions that
include a `class` parameter can convert the class of an object using
[`as_sf_class()`](https://elipousson.github.io/sfext/reference/as_sf.md).
There are a set of functions for class conversion that typically wrap
multiple sf functions with more limited input options.
...
Please note this flexibility make
[sfext](https://github.com/elipousson/sfext) easy to use (especially in
...
to get the “edges” of any geometry.
...
allows you to shift the position of an `sf` object to a new location:
...
allows you to create donuts around existing features:
...
creates a sf object with POINT geometry based on the corners of a
bounding box:
...
[sfext](https://github.com/elipousson/sfext) *depends* on two other
