...
engine_one <- read_sf_ext(url = sample_esri_url, name_col = "NAME", name = "ENGINE 1")

ggplot() +
  geom_sf(data = fire_stations) +
  geom_sf(data = engine_one, color = "red", size = 4)
```
...
- `st_transform_ext`: takes sf, sfc, or bbox objects as the crs parameter for `sf::st_transform` and supports lists of sf objects (look for the list.ok = TRUE parameter to see which functions support lists)
...
The `get_lenth()` function wraps `sf::st_length` and (for POLYGON geometries only) `lwgeom::st_perimeter`:
